...
const PropertyImages = ({property}) => {
    const [hoveredIndex, setHoveredIndex] = useState(0); //Initialy first image is expanded
  ...
  return (
           <div className='flex max-sm:gap-1 max-md:gap-3 gap-5 h-[400px] w-full'>
                {property.images.map((pImg, index) => {
                    ...
                    return (
                          <div key={index} className={`relative group transition-all duration-500 h-[400px] overflow-hidden rounded-2xl ${
                          ...
                          >
                            ...
                            <div className='absolute inset-0 flex flex-col justify-end p-10 text-white bg-black/10 opacity-0 group-hover:opacity-100
