...
const Header = () => {
   ...
   const BookingIcon = () => (
        ...
        >
             ...
             <path d="MB 21h12a2 2 0 0-2-2v-1a1 1 0 0 0-1-1H11a1 1 0 0 0-1 1v1a2 2 0 1 1-4 0V5a2 2 0 1 0-4 0v2a1 1 0 0 0 1 1h3"/>
        </svg>
   )
   ...
   useEffect(()=>{
       const handleScroll = ()=> {
          if (location.pathname === "/") {
            ...
            setActive(true) // always stay active on other pages Like         
          }
  ...
  return (
    <header className={`${active ? "bg-white py-3 shadow-md" : "py-4"} fixed top-0 w-full left-0
     right-0 z-50 transition-all duration-200`}>
        ...
        {/* Container */}
         <div className='flexBetween'>
             ...
             {/* Navbar */}
              ...
              {/* Button Searchbar and profile */}
              <div className='flex sm:flex-1 items-center sm:justify-end
               gap-x-4 sm:gap-x-8'>
                   {/* Searchbar */}
                   <div className='relative hidden xl:flex items-center'>
                       <div className={`${active ? "bg-secondary/10": "bg-white"} transition-all duration-300 ease-in-out ring-1 ring-slate-100
                       rounded-full overflow-hidden ${
                        ...
                        >
                         ...
                         <div onClick={()=> setShowSearch(prev => !prev)} className={`${active ? "bg-secondary/10": "bg-primary"} absolute right-0 ring-1 ring-slate-900/10 p-[8px] rounded-full
                         cursor-pointer z-10`}>
                   ...
                   <>
                     {menuOpened ? (
                        <img src={assets.close} alt="closeMenuIcon" onClick={toggleMenu} className={`${!active && "invert"} lg:hidden
                         cursor-pointer text-xl`}/>
                     ):(
                          <img src={assets.menu} alt="openMenuIcon" onClick={toggleMenu} className={`${!active && "invert"} lg:hidden
                         cursor-pointer text-xl`}/>
                     )}
                   </>
                   {/* User Profile */}
